...
import com.thecn.app.activities.filter.FilterActivity;
 ...
 */
public abstract class BasePostListFragment extends SwipeRefreshListFragment implements SwipeRefreshLayout.OnRefreshListener {

    private static final int FILTER_REQUEST = 0;

    protected PostsAdapter postsAdapter;
    protected PostChangeController postChangeController;
    private CallbackManager<BasePostListFragment> callbackManager;
    ...
    public PostLoader.Params postLoaderParams; //params for loading posts
    ...
    public PostLoader.MethodList postLoaderMethodList;
     ...
     */
    public static class PostCallback extends CallbackManager.NetworkCallback<BasePostListFragment> {

        public PostCallback(CallbackManager<BasePostListFragment> manager) {
            super(manager);
        }
         ...
         */
        @Override
        public void onResumeWithResponse(BasePostListFragment fragment) {
            if (!wasSuccessful()) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (posts == null) {
                        manager.addCallback(new CallbackManager.Callback<BasePostListFragment>() {
                            @Override
                            public void execute(BasePostListFragment object) {
                                //no more posts
                                object.noMore = true;
        ...
        @Override
        public void onResumeWithError(BasePostListFragment fragment) {
            fragment.onLoadingFailure();
        }
    }
     ...
     */
    public CallbackManager<BasePostListFragment> getCallbackManager() {
    ...
    private static class UserUpdater extends BroadcastReceiver {
        ...
        public UserUpdater(CallbackManager<BasePostListFragment> callbackManager) {
            this.callbackManager = callbackManager;
        }

        @Override
        public void onReceive(Context context, Intent intent) {
            callbackManager.addCallback(new CallbackManager.Callback<BasePostListFragment>() {
                @Override
                public void execute(BasePostListFragment object) {
    ...
    private static class PostChangeListener implements PostChangeController.Listener {
        ...
        private CallbackManager<BasePostListFragment> manager;

        /**
         * New instance
         * @param manager for async operations.
         */
        public PostChangeListener(CallbackManager<BasePostListFragment> manager) {
     ...
     * Uses {@link com.thecn.app.stores.PostStore#setHidePublicPosts(boolean, com.thecn.app.stores.ResponseCallback)}
     */
     ...
     * set last refresh time and use {@link PostLoader#loadPosts(BasePostListFragment, com.thecn.app.tools.network.PostLoader.Params)}
     */
    public void loadPosts() {
        ...
        postCallback = PostLoader.loadPosts(this, postLoaderParams);
    }
     ...
     */
    public abstract PostLoader.MethodList getPostLoaderMethodList();
    ...
    public void refreshForDifferentPostParams() {
        PostLoader.Params newParams = new PostLoader.Params(
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_filter) {
            ...
            intent.putExtra(FilterActivity.POST_METHODS_KEY, postLoaderMethodList);
    ...
    /**
     * When {@link com.thecn.app.activities.filter.FilterActivity} returns result,
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode != FILTER_REQUEST || resultCode != Activity.RESULT_OK || data == null) return;

        filterResult = true;

        postLoaderMethodList = data.getParcelableExtra(FilterActivity.POST_METHODS_KEY);
    }
     ...
     * Uses {@link com.thecn.app.tools.controllers.SlidingViewController} to control displaying these
     * buttons.  It should be used with {@link com.thecn.app.views.list.ObservableListView} so the amount
...
//                controller.setSlidingView(mPostButton);
     ...
     */
    private static class UpdatePubliContentsCallback extends CallbackManager.NetworkCallback<BasePostListFragment> {

        boolean hiding;

        private static final String ERROR = "Server error";

        public UpdatePubliContentsCallback(CallbackManager<BasePostListFragment> manager, boolean hiding) {
            super(manager);
            this.hiding = hiding;
        }
        ...
        @Override
        public void onResumeWithError(BasePostListFragment object) {
