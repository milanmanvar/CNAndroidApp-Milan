...
/**
 * Used to specify a methods for loading posts.  Used in {@link BasePostListFragment}.
 */
public class GobalPostLoader {
    ...
    /**
     * Params for post loader.  Contains {@link GobalPostLoader.Method} as
     * well as other values.  This class is used to specify additional flags that are set apart
     * from the loading method itself, which is specified by {@link GobalPostLoader.Method}
     */
    public static class Params {
        public GobalPostLoader.Method method;
        public int contentType;
        public boolean interpretAsGlobal;
        public boolean showGlobalPosts;

        public Params(GobalPostLoader.Method method) {
            this(method, GobalPostLoader.CONTENT_ALL);
        }

        public Params(GobalPostLoader.Method method, int contentType) {
            this.method = method;
     ...
     */
    public static BaseGlobalPostListFragment.PostCallback loadPosts(BaseGlobalPostListFragment f, Params params) {

        PostStore.ListParams storeParams = new PostStore.ListParams(f.getLimit(), f.getOffset());
        BaseGlobalPostListFragment.PostCallback callback = new BaseGlobalPostListFragment.PostCallback(f.getCallbackManager());
        ...
        switch (params.method.sourceType) {
            case SOURCE_HOME:
                ...
                break;
            case SOURCE_COURSE:
                PostStore.getGlobalPostsFromCourse(storeParams, callback);
                break;
            case SOURCE_CONEXUS:
